...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-3"><span class="text-muted fw-light">Kelas-Ku /</span> Ubah</h4>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <h5 class="card-header">Form Ubah Kelas</h5>
                    <div class="card-body">
                        <form action="{{ route('kelas.update', $kelas->id) }}" method="POST" class="form_ubah_kelas"
                            id="form_ubah_kelas">
                            @csrf
                            @method('put')
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('kelas') is-invalid @enderror" id="kelas"
                                    placeholder="Contoh: Teknik Industri(S-1) Siang-2-01"
                                    value="{{ $kelas->kelas ?? old('kelas') }}" name="kelas" />
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('id_matkul') is-invalid @enderror select2" id="id_matkul"
                                    ...
                                    @foreach (get_matakuliah() as $matkul)
                                        <option value="{{ $matkul->id }}"
                                            {{ old('id_matkul') ?? $kelas->id_matkul == $matkul->id ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('semester') is-invalid @enderror"
                                    id="semester" placeholder="Contoh: 2"
                                    value="{{ $kelas->semester ?? old('semester') }}" name="semester" />
                            </div>
                            <div class="mb-3">
                                <label for="sks" class="form-label">SKS</label>
                                <input type="number" class="form-control @error('sks') is-invalid @enderror" id="sks"
                                    placeholder="Contoh: 2" value="{{ $kelas->sks ?? old('sks') }}" name="sks" />
                            </div>
                            <div class="mb-3">
                                ...
                                <div class="table-responsive text-nowrap">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody class="data_mahasiswa">
                                            @foreach ($kelas->kelas_detail as $mahasiswa)
                                                <tr>
                                                    <td><input type="text" name="npm[]" class="form-control"
                                                            value="{{ $mahasiswa->mahasiswa->npm }}" /></td>
                                                    ...
                                                    <td>
                                                        <select name="jenis_kelamin[]" class="form-select">
                                                            <option value=""></option>
                                                            <option value="L"
                                                                {{ $mahasiswa->mahasiswa->jk == 'L' ? 'selected' : '' }}>
                                                                Laki-laki</option>
                                                            <option value="P"
                                                                {{ $mahasiswa->mahasiswa->jk == 'P' ? 'selected' : '' }}>
                                                                Perempuan</option>
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <select name="aktif[]" class="form-select">
                                                            <option value="N"
                                                                {{ $mahasiswa->aktif == 'N' ? 'selected' : '' }}>Tidak
                                                            </option>
                                                            <option value="Y"
                                                                {{ $mahasiswa->aktif == 'Y' ? 'selected' : '' }}>Ya
                                                            </option>
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <button type="button" id="btn_hapus_mahasiswa"
                                                            class="btn btn-sm btn-danger btn_hapus_mahasiswa">Hapus</button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
@push('js')
    <script>
        $(document).ready(function() {
            ...
            var count_data_mahasiswa = {{ $kelas->kelas_detail->count() }};
            ...
            $('.form_ubah_kelas').on('submit', function(event) {
                event.preventDefault();
                var form = $(this);
                $.ajax({
